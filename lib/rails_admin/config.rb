        ...
        possible =
          ...
          @@system_models ||= # memoization for tests
            ([Rails.application] + Rails::Application::Railties.engines).map do |app|
              # TODO :: add autoload_paths 
              # => Problem :: app.paths class is rails.path.paths autoload_paths is []
              # original is => (app.paths['app/models'] + app.config.autoload_paths).map do |load_path|
              (app.paths['app/models']).map do |load_path|
